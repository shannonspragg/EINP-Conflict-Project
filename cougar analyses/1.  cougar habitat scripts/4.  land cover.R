...
# Here we want to make a combined "forest" land cover, openland, developed, snow / ice, and rock / rubble rasters in addition to the others we already have
# Note: if you run this after running all the bear scripts, there is no need to re-run variables you've already made
...
st_write(ab_landcover, "data/processed/ab_landcover.shp")
...
# Make forest a continuous raster:
bhb.forest.rast[bhb.forest.rast == 210] <- 1
bhb.forest.rast[bhb.forest.rast == 220] <- 1
bhb.forest.rast[bhb.forest.rast == 230] <- 1
bhb.forest.raster <- raster(bhb.forest.rast)
bhb.forest.raster[is.na(bhb.forest.raster[])] <- 0 
names(bhb.forest.raster)[names(bhb.forest.raster) == "LC_class"] <- "forest"
