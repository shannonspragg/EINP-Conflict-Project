...
# Bring in Data: ----------------------------------------------------------
w.post.pa.full <- readRDS("data/processed/post_pa_full.rds")
w.post.pa.partial <- readRDS("data/processed/post_pa_partial.rds")
w.post.pa.full.quad <- readRDS("data/processed/post_pa_full_quad.rds")
w.post.int.only <- readRDS("data/processed/post_int_only.rds")

# Run LOOIC and Posterior Comparisons: ------------------------------------
loo1 <- loo(w.post.pa.full, save_psis = TRUE)
loo2 <- loo(w.w.post.pa.full.quad, save_psis = TRUE)
loo3 <- loo(w.post.pa.partial, save_psis = TRUE)
loo0 <- loo(w.post.int.only, save_psis = TRUE)

post_pa_loo_comp <- loo_compare(loo1, loo2, loo3, loo0)

preds <- posterior_epred(w.post.pa.full)
preds2 <- posterior_epred(w.w.post.pa.full.quad)
preds3 <- posterior_epred(w.post.pa.partial)
preds0 <- posterior_epred(w.post.int.only)
pred <- colMeans(preds)
...
ploo <- E_loo(preds, loo1$psis_object, type="mean", log_ratios = -log_lik(w.post.pa.full))$value
ploo2 <- E_loo(preds2, loo2$psis_object, type="mean", log_ratios = -log_lik(w.w.post.pa.full.quad))$value
ploo3 <- E_loo(preds3, loo3$psis_object, type="mean", log_ratios = -log_lik(w.post.pa.partial))$value
ploo0 <- E_loo(preds0, loo0$psis_object, type="mean", log_ratios = -log_lik(w.post.int.only))$value
