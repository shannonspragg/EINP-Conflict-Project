...
plot(st_geometry(conflict.conf, add=TRUE)) #1136 reports

# Generate Random Points for Pseudo-absences: -----------------------------
set.seed(2345)
p.abs.pts <- randomPoints(raster(temp.rast.bhb), 2300) # double the conflict reports
...
conf.conflict.pts.w.abs <- rbind(conflict.conf, presabs.ccs.join) # 3436 points total
