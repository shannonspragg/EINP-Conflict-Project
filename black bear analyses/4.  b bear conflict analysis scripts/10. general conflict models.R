...
conf.pres.abs$conflict_presence_pa <- 1

#filter some of the absences
pres.abs.filter <- conf.pres.abs %>% 
  dplyr::select(., c(conflict_presence_pa, dst2p_k, hum_dns, anml_fr, grnd_cr, ndvi, gHM, bhs, agn_bph, gfcl_bp, frst_s_, CCSNAME)) 
...
# Full Model:
post.pa.full <- stan_glmer(conflict_presence_ps ~ dist.2.pa.ps + human.dens.ps + animal.farm.dens.ps + ground.crop.dens.ps + ndvi.ps + gHM.ps + bhs.ps + agno.biophys.ps + gen.focal.biophys.ps + forest.sp.biophys.ps + (1 | CCSNAME.ps), 
                           data = pres.abs.scl,
